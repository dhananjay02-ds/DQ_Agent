...
class Tee(io.TextIOBase):
    """Writes to both original stdout and a buffer (so terminal output is preserved)."""
...
Example: Show me top 5 funds by growth rate.'''
    ...
    def route(self, user_query: str, callbacks=None):
        ...
        # # --- 🧠 Capture full reasoning trace printed by agents ---
        # log_buffer = io.StringIO()
        # with redirect_stdout(log_buffer):
        #     base_result = self.agent_executor.run(user_query, callbacks=callbacks)
        # full_reasoning_trace = log_buffer.getvalue().strip()  # captured terminal output
        ...
        full_reasoning_trace = log_buffer.getvalue().strip()
        base_response = base_result if isinstance(base_result, str) else str(base_result)
        ...
        return {
            ...
            "chain_of_thought_log": full_reasoning_trace
        }
